...
class IsNullSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType(IsNull::class);
    }
    ...
    public function it_calls_null(QueryBuilder $queryBuilder, ExpressionBuilder $expr)
    {
        $expression = 'a.foobar IS NULL';

        $queryBuilder->expr()->willReturn($expr);
        $expr->isNull(sprintf('%s.%s', 'a', 'foobar'))->willReturn($expression);
    ...
    public function it_uses_dql_alias_if_passed(QueryBuilder $queryBuilder, ExpressionBuilder $expr)
    {
        ...
        $queryBuilder->expr()->willReturn($expr);
        $expr->isNull(sprintf('%s.%s', 'x', 'foobar'))->willReturn('x.foobar IS NULL');

        $this->getFilter($queryBuilder, $alias)->shouldReturn('x.foobar IS NULL');
    }
}
